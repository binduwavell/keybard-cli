...
if (typeof debug !== 'undefined') {
  debugCombo = debug('keybard:combo');
} else {
  debugCombo = () => {};
}
...
async function addCombo(definitionString, options) {
  ...
  try {
    ...
    if (await USB.open()) {
      ...
      debugCombo('Attempting to set combo ID %d with keys: %o', newComboId, comboArray);
      ...
      debugCombo('Updated kbinfo.combos[%d] = %o', newComboId, comboArray);

      // Push the updated combo to the device
      await Vial.combo.push(kbinfo, newComboId);
      debugCombo('Combo push completed');
