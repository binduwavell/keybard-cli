...
let debug;
try {
  debug = require('debug')('keybard:keymap');
} catch (e) {
  debug = () => {};
}
...
async function setKeymapEntry(keyDefinition, positionStr, options) {
  ...
  try {
    ...
    if (await USB.open()) {
      ...
      debug('Setting layer %d, position %d to %s (code: 0x%s)', layerNum, keyIndex, keyDefinition, numericKeycode.toString(16));
