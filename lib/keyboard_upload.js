...
async function uploadFile(filepath, options = {}) {
  ...
  try {
    ...
    debugKeyboard('About to call USB.list()');
    const devices = USB.list();
    debugKeyboard('USB.list() returned: %o (length: %d)', devices, devices ? devices.length : 0);
    if (devices.length === 0) {
      debugKeyboard('No devices found');
      console.error("No compatible keyboard found.");
      if (process) process.exitCode = 1;
      return;
    }

    if (await USB.open(devices)) {
