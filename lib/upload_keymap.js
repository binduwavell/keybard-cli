...
async function uploadKeymap(filepath) {
  ...
  try {
    ...
    if (typeof Vial.kb.setFullKeymap !== 'function' || typeof Vial.kb.saveKeymap !== 'function') {
      console.error("Error: Vial.kb.setFullKeymap or Vial.kb.saveKeymap not found. Cannot upload keymap.");
      if (process) process.exitCode = 1;
      return;
    }
    ...
    if (await USB.open()) {
      ...
      await Vial.getKeyboardInfo(kbinfo); // To get kbinfo.rows, .cols, .layers
      ...
      console.log("Full keymap sent. Saving to keyboard's persistent memory...");
