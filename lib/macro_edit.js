...
if (typeof debug !== 'undefined') {
  debugMacroEdit = debug('keybard:macro');
} else {
  debugMacroEdit = () => {};
}
...
async function editMacro(macroIdStr, newSequenceDefinition, options) {
  ...
  try {
    ...
    if (await USB.open()) {
      ...
      debugMacroEdit('Updating macro ID %d with new actions: %o', macroId, parsedNewActions);
      debugMacroEdit('kbinfo.macros before push: %o', kbinfo.macros);
      debugMacroEdit('kbinfo.macro_count: %d, kbinfo.macros_size: %d', kbinfo.macro_count, kbinfo.macros_size);


      await Vial.macro.push(kbinfo); // Sends all macros based on the modified kbinfo.macros
      debugMacroEdit('Vial.macro.push completed');


      if (typeof Vial.kb.saveMacros === 'function') {
        await Vial.kb.saveMacros();
        debugMacroEdit('Macros saved via Vial.kb.saveMacros');
      } else {
