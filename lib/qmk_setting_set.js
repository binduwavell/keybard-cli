...
let debug;
try {
  debug = require('debug')('keybard:qmk');
} catch (e) {
  debug = () => {};
}
...
async function setQmkSetting(settingName, valueString, options = {}) {
  ...
  try {
    ...
    if (await USB.open()) {
      ...
      debug('Attempting direct set for \'%s\' with value \'%s\' (type: %s)', settingName, parsedValue, typeof parsedValue);
      if (Vial.setQmkSetting && typeof Vial.setQmkSetting === 'function') {
        attemptedMethod = "Vial.setQmkSetting";
        debug('Found %s', attemptedMethod);
