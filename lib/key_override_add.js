...
let debug;
try {
  debug = require('debug')('keybard:key-override');
} catch (e) {
  debug = () => {};
}
...
async function addKeyOverride(triggerKeyString, overrideKeyString, options = {}) {
  ...
  try {
    ...
    if (await USB.open()) {
      ...
      debug('Preparing to add override ID %d: trigger=%s(%d), override=%s(%d)', newOverrideId, triggerKeyString, parsedTriggerKey, overrideKeyString, parsedOverrideKey);
      debug('kbinfo.key_overrides before push: %o', kbinfo.key_overrides);

      await Vial.key_override.push(kbinfo, newOverrideId); // This sends the specific override to the device
      debug('Vial.key_override.push completed');
      ...
      if (typeof Vial.kb.saveKeyOverrides === 'function') {
        ...
        debug('Key overrides saved via Vial.kb.saveKeyOverrides');
      } else if (typeof Vial.kb.save === 'function') { // Fallback to a general save if it exists
        await Vial.kb.save();
        debug('Key overrides saved via Vial.kb.save');
      }
      else {
