        ...
        try {
          ...
          if (kbiData.combos && Array.isArray(kbiData.combos)) {
            ...
            if (Vial.combo && typeof Vial.combo.push === 'function') {
              try {
                ...
                // Upload each combo individually (following the pattern from combo_add.js)
                ...
                for (let comboId = 0; comboId < kbiData.combos.length; comboId++) {
                  ...
                  if (combo && Array.isArray(combo) && combo.length >= 5) {
                    // Check if combo has any non-KC_NO keys (not empty)
                    const hasValidKeys = combo.some(key => key && key !== "KC_NO" && KEY.parse(key) !== 0);
                    if (hasValidKeys) {
                ...
                console.info(`Combos from .kbi uploaded successfully. Updated ${combosUploaded} combos.`);
                sectionsSucceeded++;
              } catch (e) {
          ...
          if (kbiData.tapdances && Array.isArray(kbiData.tapdances)) {
            ...
            if (Vial.tapdance && typeof Vial.tapdance.push === 'function') {
              try {
                ...
                // Upload each tapdance individually (following the pattern from tapdance_add.js)
                ...
                for (let tdid = 0; tdid < kbiData.tapdances.length; tdid++) {
                  ...
                  if (tapdance && typeof tapdance === 'object') {
                    // Check if tapdance has any non-KC_NO actions (not empty)
                    const hasValidActions = tapdance.tap !== "KC_NO" || tapdance.hold !== "KC_NO" ||
                                          tapdance.doubletap !== "KC_NO" || tapdance.taphold !== "KC_NO";
                    if (hasValidActions) {
                ...
                console.info(`Tapdances from .kbi uploaded successfully. Updated ${tapdancesUploaded} tapdances.`);
                sectionsSucceeded++;
              } catch (e) {
          ...
          if (settingsData && typeof settingsData === 'object') {
            ...
            try {
              ...
              // Check if there's a bulk push for settings (preferred method)
              ...
              if (canPushAllSettings) {
                console.info("Using bulk settings push method for .kbi settings.");

                // Assign settings data to kbinfo first (this is what was missing!)
                ...
                // Fallback to individual setting application
                console.info("Using individual setting method for .kbi settings.");
