...
async function downloadFile(filepath, options = {}) {
  ...
  try {
    ...
    if (fileExtension !== '.svl' && fileExtension !== '.kbi') {
      console.error("Error: Invalid filepath. Output file must have a .svl or .kbi extension.");
      if (process) process.exitCode = 1;
      return;
    }
    ...
    if (await global.deviceSelection.openDeviceConnection(USB, deviceResult.device)) {
      ...
      // 3. Handle file format based on extension
      if (fileExtension === '.kbi') {
        // For .kbi files, output raw kbinfo data (like keyboard info command)
        const jsonString = JSON.stringify(kbinfo, null, 2);
        try {
          fs.writeFileSync(filepath, jsonString);
          console.log(`Keyboard info successfully downloaded to ${filepath}`);
          if (process) process.exitCode = 0;
        } catch (e) {
          console.error(`Error writing keyboard info to file "${filepath}": ${e.message}`);
          if (process) process.exitCode = 1;
        }
        USB.close();
        return;
      ...
      // 4. Construct svlData (for .svl files)
